...
| `GET /` | Charcoal LAN page. Telemetry strip is Connected (NXE packet cached), Wi-Fi dBm, and BT to smoker dBm only (no IPs, no pit/set/probes). Footer shows `FW_VERSION` (e.g. fw v1.4.0). Three states: set password, unlock, logged-in config. Session cookie after set/unlock. |
...
Do not UniFi-forward the SoftAP, `203.0.113.118`, or port 80. Do not put house Wi-Fi or the OTA token in git or `platformio.ini`.
...
Crash-fix (2026-08-27): do not call `scan->stop()` from the NimBLE advertise callback (LoadProhibited). Stop from `loop()` instead. STA-only when NVS has Wi-Fi (AP+STA + NimBLE aborts). Do not call `WiFi.setSleep(false)` with BLE on. Null-adv guard; delay scan until after BLE init. OTA soft-pauses BLE only (no NimBLE deinit in the upload callback).
