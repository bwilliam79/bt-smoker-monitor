...
Optional second radio: an ESP-32 near the smoker serves `GET /api/reading` on its LAN address. It subscribes for the 20-byte NXE notify and decodes it with the same offsets as `decode_packet` (`0000cc01` READ on this grill is the ASCII LAN IP, not temps). Join its SoftAP (`smoker-relay`) and set a **relay name**, house Wi-Fi, and OTA token at `http://192.168.4.1/` (NVS, not git). On the house IP the board accepts **name only** (`POST /name`, token required); `POST /wifi` is SoftAP-only so the house PSK is not on the STA address. `GET /health` stays unauthenticated for LAN discovery. HTTP OTA is `POST /ota` with `X-Relay-Token` (no ArduinoOTA UDP). Settings → ESP-32 relay scans the house LAN `/24` for `/health` (not Docker bridge networks) and lets you **pick** name + IP; typing an IP is only a fallback when discovery finds nothing. Live boards without a name field still appear as `smoker-relay` + IP until reflashed. The dashboard still runs in this same app — there is no second instance. Build notes and the LAN-only API are in [`firmware/esp32-relay/README.md`](firmware/esp32-relay/README.md). Do not flash the board during a live cook; the smoker allows one Bluetooth connection.

Relay radios belong in the **Log** strip, not the top of the cook page. The header is only the Connected pill (green only on a real NXE temp packet). The log strip shows Smoker IP (`/health` STA, or the grill IP once known), BT Address (`n/a (relay; MAC not published)` — the board does not expose the smoker MAC), Relay name+host, BT Signal (`bleRssi`), Wi-Fi (`wifiRssi`), poll interval, and `lastErr`. Dashes are not used; unknown fields say why. The monitor polls `/health` every tick even when `/api/reading` is 503.
