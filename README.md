...
Relay **BT Signal** on the cook UI is the ESP↔smoker Bluetooth RSSI (`bleRssi`). A **Wi-Fi** chip shows the board’s house STA RSSI (`wifiRssi`). `lastErr` from `GET /health` is shown next to those chips. The green Connected pill still means a real NXE temp packet (`/api/reading` 200), not merely `/health` up. The monitor polls `/health` every tick for those radios even when reading is 503.
