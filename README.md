...
| **Public login** | Gear → **PUBLIC LOGIN** section with real labels above fields (**Username**, **New password**). Hint: “Leave blank to keep the current password.” Credentials for `https://smoker.example.com`. Editable on kitchen LAN only (`http://203.0.113.23:8888`). Eight or more characters. Hashed in `/data/config.json`. Never returned by the API. |
...
### Public HTTPS (`smoker.example.com`)

Kitchen LAN `http://203.0.113.23:8888` stays open (no login). The public host always requires an in-app browser session:

- Unauthenticated `GET /api/*` and `WS /ws` on Host `smoker.example.com` return **401**.
...
- Caddy only reverse-proxies TLS; there is no Caddy basicauth. The ESP-32 relay (`203.0.113.118`) is not on this door.

`CORS_ORIGINS` includes `https://smoker.example.com`. `AUTH_TOKEN` is unused for this wall.

PWA install icons (`/manifest.json`, `/icon-*.png`, `/apple-touch-icon.png`, `/favicon.svg`, `/service-worker.js`) stay reachable without a session so phone home-screen install does not fall back to a theme-color letter tile. Temps APIs and `/` still require login on the public host.

### ESP-32 relay

Optional second radio: an ESP-32 near the smoker serves `GET /api/reading` on its LAN address. It subscribes for the 20-byte NXE notify and decodes it with the same offsets as `decode_packet` (`0000cc01` READ on this grill is the ASCII LAN IP, not temps). Join its SoftAP (`smoker-relay`) and set a **relay name**, house Wi-Fi, and **device password** at `http://192.168.4.1/` (NVS, not git). No default password and no minted token. On the house IP the board serves a charcoal page: radio strip (Connected, Wi-Fi dBm, BT to smoker dBm; no IPs or temps) plus set-password / unlock / logged-in config. Session cookie after unlock. No temps on that page. `POST /wifi` is SoftAP-only so the house PSK is not on the STA address. `GET /health`, `GET /api/reading`, and `GET /api/gatt` stay unauthenticated. HTTP OTA is `POST /ota` with `X-Relay-Password` (no ArduinoOTA UDP). Settings → ESP-32 relay scans the house LAN `/24` for `/health` (not Docker bridge networks) and lets you **pick** name + IP; typing an IP is only a fallback when discovery finds nothing. Live boards without a name field still appear as `smoker-relay` + IP until reflashed. The dashboard still runs in this same app — there is no second instance. Build notes and the LAN-only API are in [`firmware/esp32-relay/README.md`](firmware/esp32-relay/README.md). Do not flash the board during a live cook; the smoker allows one Bluetooth connection.

### ESP-32 case (hardware)

Print files for the NodeMCU ESP-32S relay enclosure live in [`hardware/esp32-case-v2/`](hardware/esp32-case-v2/). **v2 is the snap-fit fix Brandon printed and approved** (no screws; USB-C stays plugged in). STL/3MF for the K1C, STEP, drawings, and Cade’s generate script are in that folder.


Relay radios belong in the **Log** strip, not the top of the cook page. The header is only the Connected pill (green only on a real NXE temp packet). The log strip shows Smoker IP (grill LAN IP from BLE, never the relay STA), Relay name+host, BT Signal (`bleRssi`), Wi-Fi (`wifiRssi`), poll interval, and `lastErr`. BT Address is hidden in relay mode. Dashes are not used; unknown fields say why. The monitor polls `/health` every tick even when `/api/reading` is 503.
